...
are supported where outer-capture lookup is unambiguous. Native closure storage
also handles tested enclosing-function globs, struct destructuring and tuple-alias
rest patterns. Borrowed-capture lifetimes and some cfg-controlled forms still
have implementation gaps; these are not blanket restrictions on those syntax
categories. See [capture limits](docs/CAPTURE_LIMITS.md).
